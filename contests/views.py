...
class EditContestView(ProblemSetterRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        if contest.created_by != request.user and request.user.role != 'admin':
            messages.error(request, "You can only edit contests that you created.")
            return redirect('contests:detail', pk=contest.pk)
...
# --- NEW VIEW: EditContestProblemsView ---
class EditContestProblemsView(ProblemSetterRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        formset = ContestProblemFormSet(instance=contest)
        
        return render(request, 'contests/edit_problems.html', {
            'formset': formset,
            'contest': contest,
            'title': f'Edit Contest Problems: {contest.name}'
        })
    ...
    def post(self, request, pk):
        ...
        formset = ContestProblemFormSet(request.POST, instance=contest)
        
        if formset.is_valid():
            formset.save()
            
            messages.success(request, f"Problems for contest '{contest.name}' updated successfully!")
            return redirect('contests:detail', pk=contest.pk)
...
# Already defined at the top of the file - removing duplicate
# class CreateContestView(ProblemSetterRequiredMixin, View):
#     """
#     View for creating a new contest
#     """
...
# The following code is commented out because it was already defined at the top of the file
# This is a duplicate definition that caused errors
"""
class EditContestProblemsView(ProblemSetterRequiredMixin, View):
